...
export class CrearProductosComponent implements OnInit {
  ...
  loadCategories(): void {
    this.http.get<Category[]>('http://3.14.254.112:3000/api/category/getAll').subscribe(
      (data) => {
  ...
  loadToppings(): void {
    this.http.get<Topping[]>('http://3.14.254.112:3000/api/topping/getAll').subscribe(
      (data) => {
        this.toppings = data;
      },
      (error) => {
  ...
  loadIceCreams(): void {
    this.http.get<IceCream[]>('http://3.14.254.112:3000/api/iceCream/getAll').subscribe(
      (data) => {
        this.iceCreams = data;
      },
      (error) => {
  ...
  crearProducto(): void {
    if (this.productoForm.valid && this.selectedFile) {
      ...
      // Crear el producto
      this.http.post('http://3.14.254.112:3000/api/product/create', formData).subscribe(
        (response: any) => {
  ...
  assignToppings(productId: string, toppingIds: string[]): void {
    this.http.patch(`http://3.14.254.112:3000/api/product/${productId}/toppings`, {
      toppings: toppingIds
    }).subscribe(
  ...
  assignIceCreams(productId: string, iceCreamIds: string[]): void {
    this.http.patch(`http://3.14.254.112:3000/api/product/${productId}/iceCream`, {
