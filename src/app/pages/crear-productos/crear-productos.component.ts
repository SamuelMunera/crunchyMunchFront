...
export class CrearProductosComponent implements OnInit {
  ...
  loadCategories(): void {
    this.http.get<Category[]>('https://us-central1-crunchy-5694e.cloudfunctions.net/api/category/getAll').subscribe(
      (data) => {
  ...
  loadToppings(): void {
    this.http.get<Topping[]>('https://us-central1-crunchy-5694e.cloudfunctions.net/api/topping/getAll').subscribe(
      (data) => {
        this.toppings = data;
      },
      (error) => {
  ...
  loadIceCreams(): void {
    this.http.get<IceCream[]>('https://us-central1-crunchy-5694e.cloudfunctions.net/api/iceCream/getAll').subscribe(
      (data) => {
        this.iceCreams = data;
      },
      (error) => {
  ...
  crearProducto(): void {
    if (this.productoForm.valid && this.selectedFile) {
      ...
      // Crear el producto
      this.http.post('https://us-central1-crunchy-5694e.cloudfunctions.net/api/product/create', formData).subscribe(
        (response: any) => {
  ...
  assignToppings(productId: string, toppingIds: string[]): void {
    this.http.patch(`https://us-central1-crunchy-5694e.cloudfunctions.net/api/product/${productId}/toppings`, {
      toppings: toppingIds
    }).subscribe(
  ...
  assignIceCreams(productId: string, iceCreamIds: string[]): void {
    this.http.patch(`https://us-central1-crunchy-5694e.cloudfunctions.net/api/product/${productId}/iceCream`, {
